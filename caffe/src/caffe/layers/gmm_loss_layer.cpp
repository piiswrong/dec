...
void GMMLossLayer<Dtype>::LayerSetUp(
  ...
  N_ = this->layer_param_.gmm_loss_param().num_center();
  lambda_ = this->layer_param_.gmm_loss_param().lambda();
  ...
  if (this->blobs_.size() > 0) {
    ...
    //    this->layer_param_.gmm_loss_param().weight_filler()));
...
template <typename Dtype>
void GMMLossLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
