...
class ShufflingDataLayerTest : public MultiDeviceTest<TypeParam> {
  typedef typename TypeParam::Dtype Dtype;

 protected:
  ShufflingDataLayerTest()
      : backend_(ShufflingDataParameter_DB_LEVELDB),
  ...
  void FillLMDB(const bool unique_pixels) {
    backend_ = ShufflingDataParameter_DB_LMDB;
  ...
  void TestRead() {
    ...
    ShufflingDataParameter* shuffling_data_param = param.mutable_shuffling_data_param();
    shuffling_data_param->set_batch_size(5);
    shuffling_data_param->set_source(filename_->c_str());
    shuffling_data_param->set_backend(backend_);

    TransformationParameter* transform_param =
        param.mutable_transform_param();
    transform_param->set_scale(scale);

    ShufflingDataLayer<Dtype> layer(param);
  ...
  void TestReadShuffle() {
    ...
    ShufflingDataParameter* shuffling_data_param = param.mutable_shuffling_data_param();
    shuffling_data_param->set_batch_size(5);
    shuffling_data_param->set_buffer_factor(5);
    shuffling_data_param->set_shuffle(true);
    shuffling_data_param->set_source(filename_->c_str());
    shuffling_data_param->set_backend(backend_);
    ...
    layer.SetUp(blob_bottom_vec_, &blob_top_vec_);
    EXPECT_EQ(blob_top_data_->num(), 5);
    EXPECT_EQ(blob_top_data_->channels(), 1);
    ...
    for (int iter = 0; iter < 20; ++iter) {
      int mark[5*5];
      for (int i = 0; i < 5*5; ++i) mark[i] = 0;
      for (int i = 0; i < 5; i++) {
        layer.Forward(blob_bottom_vec_, &blob_top_vec_);
        for (int j = 0; j < 5; j++) {
          EXPECT_EQ(blob_top_label_->cpu_data()[j], blob_top_data_->cpu_data()[j]);
          EXPECT_LT(blob_top_label_->cpu_data()[j], 25);
          EXPECT_GT(blob_top_label_->cpu_data()[j], -1);
          mark[(int)blob_top_label_->cpu_data()[j]] = 1;
        }
      }
      for (int i = 0; i < 5*5; ++i) EXPECT_EQ(mark[i], 1) << "debug iter " << i;
    }
  }
  ...
  void TestReadCropTrainSequenceSeeded() {
    ...
    {
      ShufflingDataLayer<Dtype> layer1(param);
    ...
    Caffe::set_random_seed(seed_);
    ShufflingDataLayer<Dtype> layer2(param);
  ...
  virtual ~ShufflingDataLayerTest() { delete blob_top_data_; delete blob_top_label_; }

  ShufflingDataParameter_DB backend_;
...
TYPED_TEST_CASE(ShufflingDataLayerTest, TestDtypesAndDevices);

TYPED_TEST(ShufflingDataLayerTest, TestReadLevelDB) {
  const bool unique_pixels = false;  // all pixels the same; images different
  this->FillLevelDB(unique_pixels);
  this->TestRead();
}

TYPED_TEST(ShufflingDataLayerTest, TestReadShuffle) {
  this->TestReadShuffle();
}

TYPED_TEST(ShufflingDataLayerTest, TestReadCropTrainLevelDB) {
...
TYPED_TEST(ShufflingDataLayerTest, TestReadCropTrainSequenceSeededLevelDB) {
  Caffe::set_phase(Caffe::TRAIN);
  const bool unique_pixels = true;  // all images the same; pixels different
  this->FillLevelDB(unique_pixels);
...
TYPED_TEST(ShufflingDataLayerTest, TestReadCropTrainSequenceUnseededLevelDB) {
  Caffe::set_phase(Caffe::TRAIN);
  const bool unique_pixels = true;  // all images the same; pixels different
  this->FillLevelDB(unique_pixels);
...
TYPED_TEST(ShufflingDataLayerTest, TestReadCropTestLevelDB) {
...
TYPED_TEST(ShufflingDataLayerTest, TestReadLMDB) {
  const bool unique_pixels = false;  // all pixels the same; images different
  this->FillLMDB(unique_pixels);
  this->TestRead();
}

TYPED_TEST(ShufflingDataLayerTest, TestReadCropTrainLMDB) {
  Caffe::set_phase(Caffe::TRAIN);
  const bool unique_pixels = true;  // all images the same; pixels different
  this->FillLMDB(unique_pixels);
  this->TestReadCrop();
...
// Test that the sequence of random crops is consistent when using
// Caffe::set_random_seed.
TYPED_TEST(ShufflingDataLayerTest, TestReadCropTrainSequenceSeededLMDB) {
  Caffe::set_phase(Caffe::TRAIN);
  const bool unique_pixels = true;  // all images the same; pixels different
  this->FillLMDB(unique_pixels);
  this->TestReadCropTrainSequenceSeeded();
...
// Test that the sequence of random crops differs across iterations when
// Caffe::set_random_seed isn't called (and seeds from srand are ignored).
TYPED_TEST(ShufflingDataLayerTest, TestReadCropTrainSequenceUnseededLMDB) {
  Caffe::set_phase(Caffe::TRAIN);
  const bool unique_pixels = true;  // all images the same; pixels different
  this->FillLMDB(unique_pixels);
  this->TestReadCropTrainSequenceUnseeded();
...
TYPED_TEST(ShufflingDataLayerTest, TestReadCropTestLMDB) {
  Caffe::set_phase(Caffe::TEST);
  const bool unique_pixels = true;  // all images the same; pixels different
  this->FillLMDB(unique_pixels);
  this->TestReadCrop();
